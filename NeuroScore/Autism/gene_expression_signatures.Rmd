...
The ID and Epilpsy genes have been curated in-house after an in-depth literature analysis.
...
Number of cells belonging to each diagnosis (ASD or Control): 
...
* Sum of counts (i.e library size)

* Number of detected features 

* percentage of counts in the top features

Additionally, if we have given the function list of subsets, we get these quality metrics for each of those subsets as well. 


```{r QC matrixces}
per.cell <- perCellQCMetrics(sce,
                             subsets=list(ASD=ASD_genes$gene.symbol,
                                          EP_genes=Epi_genes$Gene,
                                          ID_genes=factor(ID_genes_complete$Gene)))
...
```{r colData}
plotColData(sce, x = "sum", y="detected", other_fields ="diagnosis") + facet_wrap(~diagnosis)
```
...
hist(sce$subsets_ID_genes_percent, xlab="Proportion of ID genes (%)", 
     main="", col="stateblue1", ylab="Number of cells")
hist(sce$subsets_EP_percent, xlab="Proportion of Epilepsy genes (%)", 
     main="", col="plum", ylab="Number of cells")
```

```{r}
## ep
plotColData(sce, x = "sum", y="subsets_EP_genes_percent", 
...
```{r expression calculuations, cache=TRUE}
